  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Upgrade to Centra Pro" subtitle="Master your net worth with AI finance tools">
      <div className="space-y-5">
        ...
        {/* Banner */}
        <div className="p-6 rounded-2xl bg-gray-900 border border-gray-800 text-white text-center relative">
          ...
          {/* Pricing Selector */}
          <div className="mt-4 inline-flex p-1 rounded-xl bg-black/40 border border-gray-800 text-xs font-semibold">
            <button
              onClick={() => setBillingCycle('annual')}
              className={`px-3 py-1.5 rounded-lg transition-colors ${
                billingCycle === 'annual' ? 'bg-gray-800 text-white' : 'text-gray-400'
              }`}
            >
              Annual ($4.99/mo) <span className="text-[10px] text-emerald-400 ml-1 font-bold">SAVE 40%</span>
            </button>
            <button
              onClick={() => setBillingCycle('monthly')}
              className={`px-3 py-1.5 rounded-lg transition-colors ${
                billingCycle === 'monthly' ? 'bg-gray-800 text-white' : 'text-gray-400'
              }`}
            >
              Monthly ($7.99/mo)
            </button>
        ...
        <button
          ...
          className="w-full py-3.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-sm flex items-center justify-center space-x-2 transition-colors cursor-pointer"
        >
